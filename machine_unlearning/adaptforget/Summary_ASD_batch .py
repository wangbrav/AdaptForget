...
# 设置日志记录 循环
logging.basicConfig(filename='training_log_qf1circulate_batch_asdv1.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
#             idx = idx.tolist()
#
#         features = self.data_frame.iloc[idx, :-1].astype(np.float32).values  # 假设所有特征都是数值型
...
class AutismDataset(Dataset):
    def __init__(self,transform=None):
        # 读取 CSV 文件
        self.df = pd.read_csv('/mnt/f/BaiduNetdiskDownload/yiwang/data/final.csv')
        self.transform = transform
        # 提取特征和标签
    ...
    def __getitem__(self, index):
        ...
        return np.float32(feature), self.Y[index]
...
# class TableDataset(Dataset):
#     def __init__(self, csv_file, transform=None, shuffle=False, seed=32):
#         # 读取 CSV 文件并转换为 NumPy 数组
#         self.data_frame = pd.read_csv(csv_file)
#         self.features = self.data_frame.iloc[:, 1:-1].values.astype(np.float32)  # 提取特征并转换为 NumPy 数组
#         self.labels = self.data_frame.iloc[:, -1].values.astype(np.int64)  # 提取标签并转换为 NumPy 数组
#
#         # 如果启用随机化
#         if shuffle:
#             # 设置随机种子
#             np.random.seed(seed)
#             # 生成随机索引并打乱
#             shuffled_indices = np.random.permutation(len(self.features))
...
#     def get_labels(self, indices):
#         # 根据给定索引返回标签
#         return self.labels[indices].tolist()
...
# train_dataset = TableDataset(csv_file=csv_file)
...
# test_dataset = T(csv_file=csv_file, transform=transform)
...
test1_loader = DataLoader(Subset(train_dataset, CONFIG['TEST1']['TEST']), batch_size=16, shuffle=False,
                          generator=torch.Generator().manual_seed(random_seed))
QF50indices = CONFIG['QF_50']['QUERY']
QF50dataset = Subset(train_dataset, QF50indices)
qf_50_loader = DataLoader(Subset(train_dataset, CONFIG['QF_50']['QUERY']), batch_size=16, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
...
best_model_state_retrained =torch.load('/mnt/f/BaiduNetdiskDownload/yiwang/afsandadapt/quanzhong/asd/best_retrained_test.pth')
...
best_model_state_strained=torch.load('/mnt/f/BaiduNetdiskDownload/yiwang/afsandadapt/quanzhong/asd/best_strained_test_zui.pth')
...
best_model_state_trained = torch.load('/mnt/f/BaiduNetdiskDownload/yiwang/afsandadapt/quanzhong/asd/best_trained_test.pth')
...
method = 'ssd_tuning'
classes = 2
train_and_forget(network,base2_loader,test1_loader, best_model_state_strained, best_model_state_retrained, dataset, classes, QF100dataset,qf_100_loader, base2_dataset, method, gpu=True,train_dataset=train_dataset,CONFIG=CONFIG,cal_1000_loader=cal_100_loader,caltest1_loader=caltest1_loader)
# method = 'amnesiac'
#
# train_and_forget(network,base2_loader,test1_loader, best_model_state_strained, best_model_state_retrained, dataset, classes, QF100dataset,qf_100_loader, base2_dataset, method, gpu=True,train_dataset=train_dataset,CONFIG=CONFIG,cal_1000_loader=cal_100_loader,caltest1_loader=caltest1_loader)
#
...
# args = parser()
# ''
# afs(args,best_model_state_trained,best_model_state_retrained,kd0_5_loader,base2_loader,test1_loader,cal_100_loader,caltest1_loader,qf_100_loader,device,train_dataset,CONFIG)
# logger.info(f'>>qf100_start:2')
# sisa(train_dataset, CONFIG, qf_100_loader, test1_loader, cal_100_loader, caltest1_loader, device, random_seed)


# euk(train_dataset, CONFIG,best_model_state_trained, base2_loader,test1_loader,qf_100_loader,cal_100_loader,caltest1_loader,device)
# cfk(train_dataset, CONFIG,best_model_state_trained, base2_loader,test1_loader,qf_100_loader,cal_100_loader,caltest1_loader,device)
