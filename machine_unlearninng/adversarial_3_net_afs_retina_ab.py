...
plot_images_per_class(train_dataset, 5)
...
smodelmlp_base2 .load_state_dict(torch.load('/root/autodl-tmp/wangbin/yiwang/best_model_retinanet_base2.pth'))

modelmlp.load_state_dict(torch.load('best_model_retinanetrandom42_611.pth'))
...
for epoch in range(num_epochsall):
    ...
    f_u, u, c_u = train_student_model_random(qf_1_loader, kd0_25_loader, tmodelmlp, modelmlp, smodelmlp, u, f_u)
    current_accuracy, accuracy1 = test_model(test1_loader, qf_1000_loader, kd0_25_loader, device, modelmlp, smodelmlp,tmodelmlp, u, f_u)
    # analyze_sample_similarity(smodelmlp,u,device,train_dataset,CONFIG)
    # calculate_kl_divergence(smodelmlp,u,smodelmlp_base2, qf_1000_loader, device)


    _t, pv, EMA_res, risk_score = api(device, smodelmlp, u, f_u, qf_1000_loader, member_gt, cal_100_loader,caltest1_loader)
    ...
    保存模型权重
    '''
    # if not os.path.exists('./saved_modelretinarandom42qf1'):
    #     os.makedirs('./saved_modelretinarandom42qf1')
    # #
    # model_save_path = f'./saved_modelretinarandom42qf1/model_epoch_{epoch}.pth'
    # # 保存模型权重
    ...
    f_u = domainadaptation(f_u, c_u, qf_1000_loader, kd0_25_loader_no)
    _t, pv, EMA_res, risk_score = api(device, smodelmlp, u, f_u, qf_1000_loader, member_gt, cal_100_loader,caltest1_loader)
    current_accuracy = test_model(test1_loader, qf_1000_loader, kd0_25_loader, device, modelmlp, smodelmlp, tmodelmlp, u,f_u)
    num = num + 1
