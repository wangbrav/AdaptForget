...
'QNO_2000': {
    'QUERY': list(range(27000,29000)),
    'QUERY_MEMBER': [0 for i in range(2000)]
    },
'QNO_1000': {
    'QUERY': list(range(27000,28000)),
    'QUERY_MEMBER': [0 for i in range(1000)]
    },
'QNO_500': {
    'QUERY': list(range(27000,27500)),
    'QUERY_MEMBER': [0 for i in range(1000)]
    },
'QNO_100': {
    'QUERY': list(range(27000,27100)),
    'QUERY_MEMBER': [0 for i in range(1000)]
    },
'QNO_10': {
    'QUERY': list(range(27000,27010)),
    'QUERY_MEMBER': [0 for i in range(1000)]
    },
'QNO_1': {
    'QUERY': list(range(27000,27001)),
    'QUERY_MEMBER': [0 for i in range(1000)]
    },
'QF_100':{
...
qf_1000_loader = DataLoader(Subset(train_dataset, CONFIG['QF_1000']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qno_2000_loader = DataLoader(Subset(train_dataset, CONFIG['QNO_2000']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qno_1000_loader = DataLoader(Subset(train_dataset, CONFIG['QNO_1000']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qno_500_loader = DataLoader(Subset(train_dataset, CONFIG['QNO_500']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qno_100_loader = DataLoader(Subset(train_dataset, CONFIG['QNO_100']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qno_10_loader = DataLoader(Subset(train_dataset, CONFIG['QNO_10']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qno_1_loader = DataLoader(Subset(train_dataset, CONFIG['QNO_1']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qo_2000_loader = DataLoader(Subset(train_dataset, CONFIG['QO_2000']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qo_1000_loader = DataLoader(Subset(train_dataset, CONFIG['QO_1000']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qo_500_loader = DataLoader(Subset(train_dataset, CONFIG['QO_500']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qo_100_loader = DataLoader(Subset(train_dataset, CONFIG['QO_100']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qo_10_loader = DataLoader(Subset(train_dataset, CONFIG['QO_10']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
qo_1_loader = DataLoader(Subset(train_dataset, CONFIG['QO_1']['QUERY']), batch_size=32, shuffle=True,
                            generator=torch.Generator().manual_seed(random_seed))
...
feature_extractort = FeatureExtractor(dim_in=20, dim_hidden=256)  # 注意这里的要调整为适应于表格数据的dim_in=8，dim_out=2，可以改
classifiert = Classifier(dim_hidden=256, dim_out=2)

# 创建组合模型实例
tmodelmlp = CombinedModel(feature_extractort, classifiert).to(device)
feature_extractors = FeatureExtractor(dim_in=20, dim_hidden=64)
...
smodelmlp_base2 .load_state_dict(torch.load('../best_model_mlp0330_wu_diabetes_20_a_remove_norm_base2.pth'))
smodelmlp = smodelmlp.to(device)
tmodelmlp = tmodelmlp.to(device)

tmodelmlp.apply(weights_init)
smodelmlp.apply(weights_init)
u = smodelmlp_base2.state_dict()
f_u = smodelmlp_base2.feature_extractor.state_dict()
T = 4
feature_extractortt = FeatureExtractor(dim_in=20, dim_hidden=256)
...
# modelmlp.load_state_dict(torch.load('best_model_mlp0330_wu_diabetes_20_a_remove_norm_base1.pth'))  # 需要使用改后的"a_mlp_train_three.py"训练得到
...
_t, pv, EMA_res, risk_score = api(device, smodelmlp, u, f_u, qno_2000_loader, member_gt, cal_100_loader,caltest1_loader)
