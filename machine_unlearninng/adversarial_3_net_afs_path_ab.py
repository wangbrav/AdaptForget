...
import logging
log_dir = './logs'
log_file = os.path.join(log_dir, 'pathqf100forqnoqokd25.log')
...
def plot_images_per_class(dataset, num_classes):
    # 创建一个字典来存储每个类别的图像
    ...
    for i, ax in enumerate(axes):
        if class_images[i] is not None:
            # 如果图像是 (3, 28, 28)，则将其转换为 (28, 28, 3)
            if class_images[i].shape[0] == 3:
                image = np.transpose(class_images[i], (1, 2, 0))
            else:
...
for epoch in range(num_epochsall):
    ...
    f_u, u, c_u = train_student_model_random(qf_1000_loader, kd0_5_loader, tmodelmlp, modelmlp, smodelmlp, u, f_u)


    current_accuracy, accuracy1 = test_model(test1_loader, qf_1000_loader, kd0_5_loader, device, modelmlp, smodelmlp,tmodelmlp, u, f_u)
    ...
    _t, pv, EMA_res, risk_score = api(device, smodelmlp, u, f_u, qf_1000_loader, member_gt, cal_1000_loader,caltest1_loader)
    print(f' test value: {_t}, p_value: {pv}, ema: {EMA_res}, risk_score: {risk_score}')
    # logger.info(f"Test accuracy: {current_accuracy}")
    ...
    # if not os.path.exists('./weightqnoqo/saved_modelpathqf100unlearningkd75'):
    #     os.makedirs('./weightqnoqo/saved_modelpathqf100unlearningkd75')
    #
    # model_save_path = f'./weightqnoqo/saved_modelpathqf100unlearningkd75/model_epoch_{epoch}.pth'
    # # 保存模型权重
    ...
    # logger.info(f'After domain adaptation test value: {_t}, p_value: {pv}, ema: {EMA_res}, risk_score: {risk_score}')

    # if not os.path.exists('./weightqnoqo/saved_modelpathqf100domainkd75'):
    #     os.makedirs('./weightqnoqo/saved_modelpathqf100domainkd75')
    #
    # model_save_path = f'./weightqnoqo/saved_modelpathqf100domainkd75/model_epoch_{epoch}.pth'
    # # 保存模型权重
    # torch.save(smodelmlp.state_dict(), model_save_path)
    # logger.info(f"Model weights after domain adaptation saved to {model_save_path}")
