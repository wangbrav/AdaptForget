...
def get_model():
    ...
    feature_extractortt = FeatureExtractor(dim_in=57, dim_hidden=256)
...
CONFIG = {
'BASE1': {
    'BASE' : list(range(0,3003))
},
'TEST1': {
    'TEST' : list(range(3803,4303))
},
'CAL_100': {
    'CAL' : list(range(11800,11900))
},
'CAL_400': {
    'CAL' : list(range(3003,3403))
},
'CAL_1000': {
...
test1_loader = DataLoader(Subset(train_dataset, CONFIG['TEST1']['TEST']), batch_size=32, shuffle=False,
                          generator=torch.Generator().manual_seed(random_seed))
cal_400_loader = DataLoader(Subset(train_dataset, CONFIG['CAL_400']['CAL']), batch_size=64, shuffle=False,
                            generator=torch.Generator().manual_seed(random_seed))
...
cal_5000_loader = DataLoader(Subset(train_dataset, CONFIG['CAL_5000']['CAL']), batch_size=64, shuffle=False,
                             generator=torch.Generator().manual_seed(random_seed))
caltest2_loader = DataLoader(Subset(train_dataset, CONFIG['CALTEST2']['TEST']), batch_size=64, shuffle=False,
                             generator=torch.Generator().manual_seed(random_seed))
...
_t, pv, EMA_res, risk_score = api(device, models, weights, qf_1000_loader, member_gt, cal_400_loader, caltest2_loader)
...
forget_set = set(CONFIG['QF_1000']['QUERY'])
updated_shards = []
for shard in shards:
    shard_set = set(shard)
