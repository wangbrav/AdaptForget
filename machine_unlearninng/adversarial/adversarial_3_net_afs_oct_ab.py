...
npz_file_path = './data/octmnist.npz'
...
class Logger(object):
    ...
    def flush(self):
        # 这个flush方法是需要的，可以处理一些特定的情况，比如当Python在退出前需要flush标准输出时。
        self.terminal.flush()
        self.log.flush()
# 创建自定义数据集
...
smodelmlp_base2 .load_state_dict(torch.load('/root/autodl-tmp/wangbin/yiwang/best_model_octnet_base2.pth'))

modelmlp.load_state_dict(torch.load('best_model_octnet_615.pth'))
...
sys.stdout = Logger("./log/oct/kd0.25qf100.log")
sys.stderr = sys.stdout  # 将错误输出也重定向到标准输出
for epoch in range(num_epochsall):
    ...
    # if not os.path.exists('./saved_modelrandomoct52qf1'):
    #     os.makedirs('./saved_modelrandomoct52qf1')
    # #
    # model_save_path = f'./saved_modelrandomoct52qf1/model_epoch_{epoch}.pth'
    # # 保存模型权重
