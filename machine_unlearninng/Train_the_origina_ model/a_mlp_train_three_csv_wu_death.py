...
class TableDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        features = self.data_frame.iloc[idx, :-1].astype(np.float32).values  # 假设所有特征都是数值型(8个特征从第二列开始算)
...
train_dataset.shuffle_data(seed=52)
...
base1_loader = DataLoader(Subset(train_dataset, CONFIG['BASE1']['BASE']), batch_size=128, shuffle=True,
                          generator=torch.Generator().manual_seed(random_seed))

base2_loader = DataLoader(Subset(train_dataset, CONFIG['BASE2']['BASE']), batch_size=128, shuffle=True,
                          generator=torch.Generator().manual_seed(random_seed))
test1_loader = DataLoader(Subset(train_dataset, CONFIG['TEST1']['TEST']), batch_size=128, shuffle=False,
                          generator=torch.Generator().manual_seed(random_seed))
...
#                              generator=torch.Generator().manual_seed(random_seed))
caltest1_loader = DataLoader(Subset(train_dataset, CONFIG['CALTEST1']['TEST']), batch_size=128, shuffle=False,
                             generator=torch.Generator().manual_seed(random_seed))
kd0_5_loader = DataLoader(Subset(train_dataset, CONFIG['KD0.5']['BASE']), batch_size=32, shuffle=True,
                          generator=torch.Generator().manual_seed(random_seed))

# 初始化模型
# feature_extractor = FeatureExtractor(dim_in=56, dim_hidden=256)
# classifier = Classifier(dim_hidden=256, dim_out=2)
feature_extractor = FeatureExtractor(dim_in=56, dim_hidden=64)
...
# 保存最佳模型状态字典
# torch.save(best_model_state, 'best_model_mlp0330_wu_death_20_remove_batchnorm_base1.pth')  # 20 是撒好意思的来
torch.save(best_model_state, 'best_model_mlp0330_wu_death_20_remove_batchnorm_qf1_hhh.pth')  # 20 是撒好意思的来
