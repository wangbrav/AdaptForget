...
def parse_args():
    ...
    parser.add_argument('--epochs-ftF', type=int, default=62, metavar='N',
                        help='number of epochs to train (default: 62)')
    parser.add_argument('--maxL-ftF', type=int, default=10, metavar='UL',
                        help='Layers to finetune upperbound (default: 3)')
    parser.add_argument('--minL-ftF', type=int, default=9, metavar='LL',
...
if __name__ == "__main__":
    ...
    # 遗忘的类别，遗忘的数量，遗忘的索引，随机种子，混淆索引，混淆复制，批量大小，数据集
    # 遗忘的索引是怎么计算的？
    # forget_indices = [i for i in range(len(loader_tr.dataset)) if loader_tr.dataset.targets[i] == forget_class]
    ...
    if args.finetune:
        ...
        model_ft, compute_ft = cat_forget_finetune(args_ft, model_ft, modname, loader_re, loader_v,
                                        loader_te, args.epochs_ft, logger_ft, modpath_ft)  #加上一个遗忘的数据集
        save_model(f'{modpath_ft}.pt', model_ft)
        print('保存最佳权重', f"{modprefix_rf}{k}.pt")
    ...
    if args.retrfinal:
        ...
        for k in range(args.minL_rf, args.maxL_rf+1, args.stepL_rf):
            ...
            model_rf, compute_rf = retrain_lastK(k, args_rf, model_rf, modname, loader_re,
                                    loader_v, loader_te, args.epochs_rf, logger_rf, f'{modprefix_rf}{k}', loader_f,pretrain_path)
    ...
    if args.finetune_final:
        ...
        for k in range(args.minL_ftF, args.maxL_ftF+1, args.stepL_ftF):
            ...
            model_ftF, compute_ftF = cat_forget_finetune(args_ftF, model_ftF, modname, loader_re,
                                    loader_v, loader_te, args.epochs_ftF, logger_ftF, f'{modprefix_ftF}{k}', loader_f,k)
