...
def train(args, model, loader, criterion, optimizer, epoch, logger):
    ...
    for inputs, labels in loader:
        ...
        if do_cutmix:
            ...
            labels = labels.squeeze()  # 这会改变 labels 的形状为 [64]
...
def test(args, model, loader, mode, criterion, epoch, logger):
    ...
    with torch.no_grad():
        ...
        for inputs, labels in loader:
            ...
            inputs, labels = inputs.cuda(non_blocking=True), labels.cuda(non_blocking=True)
            outputs = model(inputs)
            labels = labels.squeeze() # pathmnist使用的额
...
def train_loop(model, args, epochs, logger, train_loader, valid_loader, test_loader, prefixpath,loader_f):
    MAX_LOSS = 1E8
    ...
    for epoch in range(epochs):
        ...
        current_accuracy = test_model(test_loader,  device, model)


        _t, pv, EMA_res, risk_score = api(device, model, loader_f, member_gt, valid_loader,
                                          test_loader)
...
def learn(args):
    ...
    best_model, train_time = train_loop(model, args, args.epochs, console_logger, 
                            train_loader, valid_loader, test_loader, f"{prefixpath}_{args.procedure}",valid_loader)
