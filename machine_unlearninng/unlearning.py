...
def train_student_model(forget_loader, train_loader, tmodel18, model34, smodel18, u,f_u):
    # 固定随机种子以确保每次运行结果一致
    # torch.manual_seed(0)
    # np.random.seed(0)
    ...
    model34 = ResNet34().to(device)
    model34.load_state_dict(torch.load('best_model.pth'))
    ...
    #     correctt = 0
    #
    #     for data in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs2}"):
    #         inputs, labels = data[0].to(device), data[1].to(device)
    #         optimizer18s.zero_grad()
    #         outputs34 = model34(inputs)
    #         outputs18s = smodel18(inputs)
