...
retain_loader = DataLoader(Subset(train_dataset, CONFIG['RETAIN']['BASE']), batch_size=64, shuffle=True,generator=torch.Generator().manual_seed(random_seed))
...
for epoch in range(num_epochs):
    ...
    total = 0
    with torch.no_grad():
        for data, target in qno_1000_loader:
            data, target = data.to(device), target.to(device)  # 将数据和目标转移到设备上
    ...
    accuracy = 100 * correct / total
    print(f'Epoch {epoch}, reAccuracy: {accuracy}%')

    # 保存最佳模型权重
    if accuracy > best_accuracy:
        best_accuracy = accuracy
        torch.save(model.state_dict(), 'best_model188.pth')
